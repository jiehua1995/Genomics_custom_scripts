#!/usr/bin/env python3
"""
file_checksum.py - Calculate file hashes for data integrity verification

Usage:
    file-checksum -p /path/to/directory -o hashes.txt -a sha256

Description:
    Recursively scans a directory and calculates hash values for all files.
    Outputs results to a tab-separated file with filename and hash pairs.
    Supports multiple hash algorithms: md5, sha1, sha256, sha224, sha512.
"""
...
def write_hashes_to_file(directory, output_file, hash_algorithm):
    """
    Calculate the hash of all files in the directory and write the relative paths and hashes to a text file.
    ...
    with open(output_file, "w") as f:
        ...
        f.write("# File Path (relative)\tHash Value\n")  # Header with relative path info
        
        # Create progress display - use tqdm if available, otherwise simple counter
        if HAS_TQDM:
        ...
        for i, file_path in enumerate(iterator):
            ...
            try:
                ...
                # If relative path fails (e.g., different drives on Windows), use full path
                relative_path = file_path
            
            # Calculate the hash of the file using the specified algorithm
            file_hash = calculate_hash(file_path, hash_algorithm)
            
            # Write the relative path and its corresponding hash to the output file
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate file hashes and output to a text file.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  file-checksum -p ./data -o checksums.txt
  file-checksum -p /path/to/files -o hashes.txt -a md5
  file-checksum -p ./genome_data -o integrity.txt -a sha512
        """
    )
    parser.add_argument("-p", "--path", required=True, 
                       help="[REQUIRED] Path to the directory to scan for files")
    parser.add_argument("-o", "--output", required=True, 
                       help="[REQUIRED] Output file to save the hashes and file names")
    parser.add_argument("-a", "--algorithm", default="sha256", 
                       choices=["md5", "sha1", "sha256", "sha224", "sha512"],
                       help="[OPTIONAL] Hash algorithm to use (default: sha256). Options: md5, sha1, sha256, sha224, sha512")
